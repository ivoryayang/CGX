#include "common.glsl" // Include common functions/constants
#iChannel0 "self"

#define INFINITY 1.0e30 // Define infinity constant
#define DIFFUSE 0      // Define material type constants
#define METAL 1
#define DIELECTRIC 2
...
struct HitRec {
    ...
    bool front_face; // Indicates if the ray hit the front face of an object
};
...
void setFaceNormal(inout HitRec rec, const Ray r, const vec3 outward) {
    rec.front_face = dot(r.dir, outward) < 0.0; // Determine if the ray hit the front face
    rec.norm = rec.front_face ? outward : -outward; // Set the normal direction accordingly
}
...
vec3 scatter(const Mat mat, const vec3 dir, const vec3 norm, inout vec3 att, bool front_face) {
    vec3 scattDir;
    if (mat.type == DIFFUSE) {
