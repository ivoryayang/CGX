...
// ################
    ...
    var current_p = this.parent;
    var parents = [];

    // While parent is not null, apply pose matrix of parent
    while (current_p != null) {
        parents.push(current_p);
        current_p = current_p.parent;
    }
    
    // Apply pose matrices of parent hierarchy in reverse order
    // Translation and rotation, but no scaling
    for (var i = parents.length ; i > 0 ; i--) {
        var translation_2 = SimpleMatrix.translate(parents[i-1].position[0], parents[i-1].position[1], parents[i-1].position[2]);
        var rotation_2 = rotateAroundAxisAtPoint(parents[i-1].jointAxis, parents[i-1].jointAngle, parents[i-1].jointLocation);
        pose = pose.multiply(translation_2).multiply(rotation_2);
    }

    // Apply pose matrix of current bone
    var translation = SimpleMatrix.translate(this.position[0], this.position[1], this.position[2]);
    // Axis, location, angle
    var rotation = rotateAroundAxisAtPoint(this.jointAxis, this.jointAngle, this.jointLocation);
    pose = pose.multiply(translation).multiply(rotation);

    // Return resulting pose matrix
    return pose;
}
...
// ################
    // Compute pose matrix
    var matrix = this.computePoseMatrix();
    // Apply scaling
    var scale = SimpleMatrix.scale(this.scale[0], this.scale[1], this.scale[2]);
    // Multiply matrices for final model matrix
    pose = pose.multiply(matrix).multiply(scale);

    // Return resulting model matrix
    return pose;
}
